CLASS zcl_lk22_ui5_ticket DEFINITION PUBLIC CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      BEGIN OF ty_s_ticket,
        title      TYPE zlk22_title,
        priority   TYPE zlk22_priority,
        status     TYPE zlk22_status,
...
CLASS zcl_lk22_ui5_ticket IMPLEMENTATION.
  ...
  METHOD on_event_create.
    ...
    MODIFY ENTITIES OF zi_lk22_ticket
      ENTITY Ticket
    ...
    COMMIT ENTITIES RESPONSE OF zi_lk22_ticket
  ...
  METHOD view_display.
    ...
    page->simple_form( editable = abap_true
        ...
        )->input( value = client->_bind( ms_create-title )
        )->label( `Priority (H / M / L)`
        )->input( value = client->_bind( ms_create-priority )
        )->label( `Status`
        )->input( value = client->_bind( ms_create-status )
        )->button(
            text  = `Create Ticket`
