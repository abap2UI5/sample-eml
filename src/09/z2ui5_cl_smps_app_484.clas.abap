  ...
  METHOD z2ui5_if_app~main.
    ...
    IF client->check_on_init( ).
      ...
      page->ele( n = `SimpleForm` ns = `form`
                  ...
                  )->a( n = `text` v = `Product (received navigation parameter)` )->tag( `Input`
                  )->a( n = `enabled` b = abap_false
                  )->a( n = `value`   v = client->_bind( product ) )->tag( `Label`
                  )->a( n = `text` v = `Quantity (received navigation parameter)` )->tag( `Input`
                  )->a( n = `enabled` b = abap_false
