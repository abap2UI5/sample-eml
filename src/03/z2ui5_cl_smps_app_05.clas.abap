...
CLASS z2ui5_cl_smps_app_05 IMPLEMENTATION.
  ...
  METHOD view_display.
    ...
    table->ele( `headerToolbar` )->ele( `Toolbar` )->tag( `Title`
                )->a( n = `text` v = `Travels (Z2UI5_R_SMPS_TRV)` )->tag( `ToolbarSpacer` )->tag( `Button`
                )->a( n = `press` v = client->_event( `CREATE` )
                )->a( n = `text`  v = `Create`
                )->a( n = `icon`  v = `sap-icon://add`
                )->a( n = `type`  v = `Emphasized` )->tag( `Button`
                )->a( n = `press` v = client->_event( `GENERATE` )
                )->a( n = `text`  v = `Generate Demo Data` )->tag( `Button`
  ...
  METHOD popup_create_display.
    ...
    dialog->ele( n = `SimpleForm` ns = `form`
            ...
            )->a( n = `value`       v = client->_bind( s_create-agency_id ) )->tag( `Label`
            )->a( n = `text` v = `Customer ID` )->tag( `Input`
            )->a( n = `placeholder` v = `e.g. 1`
            )->a( n = `value`       v = client->_bind( s_create-customer_id ) )->tag( `Label`
            ...
            )->a( n = `value`       v = client->_bind( s_create-booking_fee ) )->tag( `Label`
            )->a( n = `text` v = `Currency` )->tag( `Input`
            )->a( n = `value` v = client->_bind( s_create-currency ) )->tag( `Label`
